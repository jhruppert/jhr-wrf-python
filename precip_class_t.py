...
# Input:
# 
#       Q_INT: 4D array of vertically integrated hydrometeors as f(q, t, x1, x2), where
#               q(5) is the hydrometeor dimension, arranged as
#               ['QCLOUD', 'QRAIN', 'QICE', 'QSNOW', 'QGRAUP'].
# 
# Returns:
# 
#       C_TYPE: 3D array as f(t, x1, x2) with classification results:
#               0: non-precipitating
#           Convective:
#               1: deep convective
#               2: congestus
...
def precip_class_t(q_int):

    nt = q_int.shape[0]
    nx1,nx2 = q_int.shape[2:4]
    ...
    for it in range(nt): # loop through lat
        for ix1 in range(nx1): # loop through lat
            for ix2 in range(nx2): # loop through lon
                ...
                if TWP[it,ix1,ix2] > twp_thresh:

                    # Skip points with zero CWP (NANs)
                    if LWP[it,ix1,ix2] != 0:

                        CR = IWP[it,ix1,ix2]/LWP[it,ix1,ix2] # cloud ratio

                        # Convective types
                        if CR <= cr_thresh: # bottom-heaviness threshold

                            if q_int[1,it,ix1,ix2] >= rain_thresh_conv: # rain threshold
                                if q_int[4,it,ix1,ix2] >= graup_thresh: # graupel threshold
                                    c_type[it,ix1,ix2] = 1 # deep convective
                                else:
                                    c_type[it,ix1,ix2] = 2 # congestus convective
                            else:
                                c_type[it,ix1,ix2] = 3 # shallow

                        # Stratiform types
                        else:
                            if q_int[1,it,ix1,ix2] >= rain_thresh_strat: # rain threshold
                                c_type[it,ix1,ix2] = 4 # stratiform
                            else:
                                c_type[it,ix1,ix2] = 5 # anvil
