...
def read_all_vars(datdir, t0, t1, dims_invar, proc_var_list):
    ...
    dims_allvars = (nvars,dims_invar[0],dims_invar[1],dims_invar[2],dims_invar[3])
    allvars = np.ndarray(dims_allvars, dtype=np.float64)

    # if comm.rank == 0:
    varname = 'T'
    allvars[0,...] = var_read_3d(datdir,varname,t0,t1,mask=True,drop=True) # K
    # elif comm.rank == 1:
    varname = 'QVAPOR'
    allvars[1,...] = var_read_3d(datdir,varname,t0,t1,mask=True,drop=True) # kg/kg
    # elif comm.rank == 2:
    varname = 'rho'
    allvars[2,...] = read_mse_diag(datdir,varname,t0,t1,mask=True,drop=True) # kg/m3
    # else:
    for ivar in range(3,nvars):
        allvars[ivar,...] = var_read_3d(datdir,proc_var_list[ivar],t0,t1,mask=True,drop=True) # K/s or m/s

    return theta_e, pclass_z, allvars

################################

def run_binning(ipclass, bins, theta_e, allvars, pclass_z):

    # Mask out based on precipitation class
    if (ipclass > 0):
        indices = (pclass_z != ipclass)
        indices_vars = (np.repeat(pclass_z[np.newaxis,...],nvars,axis=0) != ipclass)
        theta_e_masked = np.ma.masked_where(indices, theta_e, copy=True)
        allvars_masked = np.ma.masked_where(indices_vars, allvars, copy=True)
    else:
        # Create memory references
        theta_e_masked = theta_e
        allvars_masked = allvars

    theta_e_mean = np.ma.mean(theta_e_masked, axis=(2,3))

    # Bin the variables from (x,y) --> (bin)

    dims_allvars = allvars.shape
    dims = (dims_allvars[0],dims_allvars[1],dims_allvars[2],nbins-1)
    dims_freq = (dims_allvars[1],dims_allvars[2],nbins-1)
    allvars_binned = np.full(dims, np.nan)
    freq_binned = np.ndarray(dims_freq, dtype=np.float64)
            ...
            for ibin in range(nbins-1):
                ...
                if binfreq > nmin:
                    for ivar in range(nvars):
                            allvars_binned[ivar,it,iz,ibin] = np.ma.mean(allvars_masked[ivar,it,iz,indices[0],indices[1]])

    return freq_binned, allvars_binned, theta_e_mean

################################

def driver_loop_write_ncdf(datdir, bins, t0, t1, dims_invar, proc_var_list):

    # Read variables
    theta_e, pclass_z, allvars = read_all_vars(datdir, t0, t1, dims_invar, proc_var_list)
    ...
    for ipclass in range(npclass):
        ...
        freq_binned, allvars_binned, theta_e_mean = run_binning(ipclass,bins,theta_e,allvars,pclass_z)
        ...
        for ivar in range(nvars):
            var_list_write.append(allvars_binned[ivar])
...
# for ktest in range(1):
    ...
    # for imemb in range(2,3):
        ...
        dims_invar = (nt,nz,nx1,nx2)

        driver_loop_write_ncdf(datdir, bins, t0, t1, dims_invar, proc_var_list)
