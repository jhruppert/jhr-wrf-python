...
#            indices = ((var_all[:,:,iz,:,:] >= bins[ibin]-hstep) & (var_all[:,:,iz,:,:] < bins[ibin]+hstep)).nonzero()
  ...
  for ktest in range(ntest):
    ...
    im = ax.contourf(bin_axis, pres, pltvar, clevs, norm=matplotlib.colors.LogNorm(), \
    ...
    locmin = matplotlib.ticker.SymmetricalLogLocator(base=10.0,linthresh=2,subs=np.arange(2,11,2)*0.1)
    ...
    cbar = plt.colorbar(im, ax=ax, shrink=0.75) #, ticks=clevs, format=ticker.LogFormatterMathtext())
    cbar.ax.set_ylabel('%')
  ...
  im = ax.contourf(bin_axis, pres, pltvar, clevs, norm=matplotlib.colors.SymLogNorm(linthresh=clevsi[0],linscale=clevsi[0]), \
