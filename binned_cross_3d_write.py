...
def write_nc(file_out,nt,nz,nbins,pres,bin_axis,var_binned,ivar_mean,
    ...
    binsv.units = bin_units
    binsv.long_name = 'bin_axis'
    binsv[:] = bin_axis

    vmf_binned = ncfile.createVariable(v_label, np.float64, ('nt','nz','nbins',))
    vmf_binned.units = v_units
    vmf_binned.long_name = v_descrip
    vmf_binned[:,:,:] = var_binned

    meanv = ncfile.createVariable(ivmean_label, np.float64, ('nt','nz',))
    meanv.units = ivmean_units
    meanv.long_name = ivmean_descrip
    meanv[:,:] = ivar_mean
...
# Vertical mass flux
    ...
    ivmean_descrip='vertical mass flux averaged over x,y'
...
if avgvar_select == 'vmf':
    ...
    v_units='kg / m2 / s'
    v_descrip='vertical mass flux averaged over bin,x,y'
# Theta-e (equivalent potential temperature)
elif avgvar_select == 'th_e':
    v_label=avgvar_select
    v_units='K'
    v_descrip='equiv potential temperature averaged over bin,x,y'
